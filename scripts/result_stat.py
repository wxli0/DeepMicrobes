"""
Calculates the average precision, average recall and incorrect rate for Task 1\
...
from scripts.config import MT_MAT_path
import config
import os
import pandas as pd


paths = [os.path.join(config.MT_MAT_path, 'outputs-r202/MLDSP-prediction-full-path.csv'),\
    os.path.join(config.MT_MAT_path, 'outputs-HGR-r202/HGR-prediction-full-path.csv')]
...
path1 = os.path.join(config.MT_MAT_path, 'outputs-r202/MLDSP-prediction-full-path.csv')
...
precision, recall, incorrect, rejection_rate = calc_pr(path1, res_path1, ignore_indices=ignore_indices1)
print("==== GTDB result ====")
