...
for subdir in os.listdir(base_path+dir):
    with open(os.path.join(base_path, combined_dir, subdir+"_combined.fna"), 'wb') as outfile:
...
label_id = 0
with open(os.path.join(base_path,label_dir,"label_"+dir+".txt"), 'w') as f:
    for fna_file in os.listdir(os.path.join(base_path, combined_dir)):
...
label_id=0
with open(os.path.join(base_path, "data/name2label_"+dir+".txt"), "w") as f:
    for fna_file in os.listdir(os.path.join(base_path, combined_dir)):
